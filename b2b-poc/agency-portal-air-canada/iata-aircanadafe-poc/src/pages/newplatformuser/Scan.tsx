...
import { NavigateFunction, useLocation, useNavigate } from "react-router-dom"; // Import useHistory
...
const Scan = () => {
    ...
    if (newCredId) {
      const interval = setInterval(async () => {
        try {
          ...
          if (
            ...
            navigate("/scanning", {
              state: { credProofId: newCredId, screen: "employee-signup" },
            });
          }
        } catch (error) {
      ...
      if (credProofId) {
        const interval = setInterval(async () => {
          try {
            const response: IAgencyResponse = await API.get(
              `/api/auth/agencies/employee/${credProofId}`,
            );
            console.log(response, "API response");
            if (
              response.success &&
              ...
              clearInterval(interval);
              navigate("/scanning", {
                state: { credProofId, screen: "employee-signup" },
              });
            }
          } catch (error) {
            console.error("Error fetching offer status:", error);
  ...
  }, [credProofId, navigate, newCredId]);

  //handle Generate new QR code click
  const generateNewQrCode: () => Promise<void> = async (): Promise<void> => {
    setHandleReGeneratingQrCode(true);
    const RPRequestResponse: IRequestQRResponse =
      await API.post("/api/proof-request");
    if (RPRequestResponse.data) {
      setNewQrCode(RPRequestResponse.data.longUrl);
      setNewCredId(RPRequestResponse.data.credProofId);
      setHandleReGeneratingQrCode(false);
    }
  };
  return (
            ...
            <div className="flex flex-col mr-12 ">
              <div className="mb-12">
                ...
                <p className="text-sm text-gray-600">
                  Use your phone’s camera to scan the QR code. Once scanned,
                  you’ll be directed to the next step.
                </p>
              </div>
              {/* Buttons Container */}
              <div className="flex gap-x-4">
                {/* Back to Log in Button */}
                <button className="relative text-redPrimary w-32 bg-[#E2E2E2] rounded-md overflow-hidden group">
                  {/* Hover Effect */}
